  ...
  return (
    <div className="year-block">
      <div className="year-header" onClick={() => onToggle(year.yearIndex)}>
        <div className="year-title">{year.title ? year.title : (year.yearIndex === 0 ? 'Semestre Actual' : `Año Simulado ${year.yearIndex}`)}</div>
        <div className="year-toggle">{expanded ? '−' : '+'}</div>
      </div>
      {expanded && (
        <div className="year-content">
          {(() => {
            const semestersToShow = year.yearIndex === 0 ? [year.semesters[year.semesters.length - 1]] : year.semesters;
            return semestersToShow.map((s: SemesterSim, si: number) => (
